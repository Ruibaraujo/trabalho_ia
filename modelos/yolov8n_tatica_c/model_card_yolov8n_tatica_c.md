# Model Card — YOLOv8n Tática C (v1.0.0)

## 1. Overview
- **Goal:** Testar hiperparametros, o uso do otimizador `SGD` com learning rate inicial `lr0=0.001`, para verificar qual das taticas é a melhor para o treino dos modelos. Com o fim de ,onitorizar e auditar autonomamente o estado e a conformidade das luvas dos operadores em bancadas de trabalho farmacêuticas e laboratórios, prevenindo contaminações cruzadas de substâncias e salvaguardando a integridade física dos trabalhadores contra agentes químicos nocivos.
...
|-----------------------|--------|--------|--------|--------|-------------|
| `glove_not_fitted`    | 0.956  | 0.917  | 0.936  | 0.975  | 0.854       |
| `glove_off`           | 0.997  | 1.000  | 0.998  | 0.995  | 0.909       |
| `glove_on`            | 0.999  | 1.000  | 0.999  | 0.995  | 0.979       |
| `glove_partial`       | 0.996  | 1.000  | 0.998  | 0.995  | 0.937       |
| `gloves_contaminated` | 0.998  | 1.000  | 0.999  | 0.995  | 0.979       |
| `gloves_damaged`      | 0.997  | 1.000  | 0.999  | 0.995  | 0.954       |

- **Qualitative analysis:** A matriz de confusão normalizada revela que cinco das seis classes atingem 1.00 na diagonal, sem qualquer confusão entre classes — todos os erros residem exclusivamente na linha e coluna de background. A classe `glove_not_fitted` mantém Recall de 0.917. É o modelo com o `mAP@0.5` mais elevado da Fase 1 (0.9916) e o único onde os falsos positivos de background se distribuem de forma mais equilibrada entre três classes: 50% `gloves_contaminated`, 25% `glove_not_fitted` e 25% `glove_partial`. Esta combinação — mAP mais alto e ausência de confusão entre classes — torna a Tática C a mais favorável para contexto farmacêutico, onde falsos positivos são preferíveis a falsos negativos.
- **Recommended confidence threshold(s):** O limiar ótimo segundo a curva F1 é `conf=0.656`, onde o modelo atinge F1=0.99. Para deployment operacional, o threshold `conf=0.40` pode ser preferível em contexto de segurança industrial para maximizar recall.
...
- **Padrões de Erro:** A classe `glove_not_fitted` apresenta 8% de falsos negativos (Recall 0.917), sem qualquer confusão entre classes. Os falsos positivos de background distribuem-se entre três classes: 50% `gloves_contaminated`, 25% `glove_not_fitted` e 25% `glove_partial` — a distribuição mais equilibrada da Fase 1. Nos testes de inferência com imagens externas ao dataset, o modelo não detetou a classe `gloves_contaminated` em pelo menos um caso, devolvendo lista vazia, o que evidencia limitações de generalização fora da distribuição de treino.

## 6. Deployment Notes
- **Input requirements:** Resolução estrita de entrada de $640 \times 640$ píxeis, mapeamento em formato de três canais RGB.
- **Output format:** Ficheiro estruturado em lote contendo matrizes com o formato de coordenadas $[x_1, y_1, x_2, y_2]$, identificadores inteiros de classe (`category_id`), rótulos descritivos e scores probabilísticos de confiança $[0.0, 1.0]$. Recomendado o limiar operacional de `conf=0.656`, correspondente ao pico da curva macro F1. A latência de inferência não foi medida experimentalmente neste ciclo de desenvolvimento, sendo recomendada a realização de um benchmark no hardware alvo antes de qualquer deployment em produção.
