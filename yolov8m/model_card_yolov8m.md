...
- **Métricas por Classe:**
  ...
  |-----------------------|--------|--------|--------|--------|-------------|
  | `glove_not_fitted`    | 0.995  | 0.917  | 0.954  | 0.949  | 0.828       |
  | `glove_off`           | 0.999  | 1.000  | 1.000  | 0.995  | 0.907       |
  | `glove_on`            | 0.999  | 1.000  | 1.000  | 0.995  | 0.982       |
  | `glove_partial`       | 0.999  | 1.000  | 1.000  | 0.995  | 0.933       |
  | `gloves_contaminated` | 0.996  | 0.997  | 0.997  | 0.995  | 0.983       |
  | `gloves_damaged`      | 0.999  | 1.000  | 1.000  | 0.995  | 0.957       |

- **F1-Score Global (@ confiança ótima 0.835 — pico da curva F1):** 0.9918
- **Qualitative analysis:** A matriz de confusão normalizada confirma excelente discriminação em `glove_off`, `glove_on`, `glove_partial`, `gloves_contaminated` e `gloves_damaged`, todas com 1.00 na diagonal. A principal limitação é na classe `glove_not_fitted` (Recall 0.917), onde 8% das instâncias escapam para background. Observam-se falsos positivos relevantes: 67% do background é classificado como `gloves_contaminated`, e 33% como `glove_not_fitted`, indicando sensibilidade excessiva do modelo a regiões sem luva.
- **Recommended confidence threshold(s):** O limiar ótimo segundo a curva F1 é `conf=0.835`, onde o modelo atinge F1=0.99 para todas as classes. Para deployment operacional, o threshold `conf=0.40` pode ser preferível em contexto de segurança industrial, onde é mais crítico maximizar o recall (detetar em excesso) do que minimizar falsos positivos.
...
- **Padrões de Erro:** A matriz de confusão do conjunto de teste revela que a classe `glove_not_fitted` apresenta 8% de falsos negativos, com instâncias a escapar para background. Adicionalmente, 67% das regiões de background são erroneamente classificadas como `gloves_contaminated`, e 33% como `glove_not_fitted`. Existe ainda uma ligeira tendência para falsos negativos em objetos muito afastados da câmara ou afetados por *motion blur* induzido pelo movimento rápido das mãos do operador.

## 6. Deployment Notes
- **Input requirements:** Resolução estrita de entrada de $640 \times 640$ píxeis, mapeamento em formato de três canais RGB.
- **Output format:** Ficheiro estruturado em lote contendo matrizes com o formato de coordenadas $[x_1, y_1, x_2, y_2]$, identificadores inteiros de classe (`category_id`), rótulos descritivos e scores probabilísticos de confiança $[0.0, 1.0]$. Recomendado o limiar operacional de `conf=0.835`, correspondente ao pico da curva macro F1 onde o modelo atinge F1=0.99 para todas as classes. A latência de inferência não foi medida experimentalmente neste ciclo de desenvolvimento, sendo recomendada a realização de um benchmark no hardware alvo antes de qualquer deployment em produção.
